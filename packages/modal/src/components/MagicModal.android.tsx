...
/**
 * Android-specific implementation that addresses Issue #155
 * This version includes timing adjustments to prevent crashes when navigating
 * immediately after hiding the modal.
 */
export const MagicModal = memo(
  ({
    ...
    // Android-specific: Use minimal animation timing to prevent navigation crashes
    // This is a workaround for Issue #155
    const safeAnimationInTiming = config.animationInTiming === undefined ? 1 : config.animationInTiming;
    const safeAnimationOutTiming = config.animationOutTiming === undefined ? 1 : config.animationOutTiming;
    ...
    return (
      <View style={[StyleSheet.absoluteFill, styles.pointerEventsBoxNone]}>
        <Animated.View
          ...
          entering={FadeIn.duration(safeAnimationInTiming)}
          exiting={FadeOut.duration(safeAnimationOutTiming)}
            ...
            entering={
              !isSwipeComplete
                ? (config.entering ??
                  ...
                  ].duration(safeAnimationInTiming))
                : undefined
            }
            exiting={
              !isSwipeComplete
                ? (config.exiting ??
                  defaultAnimationOutMap[
                    config.swipeDirection ?? defaultDirection
                  ].duration(safeAnimationOutTiming))
                : undefined
            }
          >
